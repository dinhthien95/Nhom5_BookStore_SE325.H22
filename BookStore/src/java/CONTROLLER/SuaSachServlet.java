...
@WebServlet(name = "SuaSachServlet", urlPatterns = {"/SuaSachServlet"})
public class SuaSachServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Double.parseDouble(gia_ban) == 0) {
            loi += "Nhập lại giá bán.";
        }
        if (loi.length() > 0) {
            request.setAttribute("loi", loi);
        }
        String url = "/sua_sach.jsp";
        try {
            if (loi.length() == 0) {
                sachDAO.updateProduct(new Sach(Integer.parseInt(ma_sach),
                        Integer.parseInt(ma_the_loai), 
                        ten_sach,
                        hinh_anh,
                        Double.parseDouble(gia_ban),
                        nha_xuat_ban, 
                        mo_ta,
                        tac_gia));
